<template>
  <div class="index">
    <Card>
      <Form ref="req" :model="req" :label-width="120" inline>
        <FormItem label="快速查询" prop="relationship">
          <Select v-model="req.relationship" placeholder="" clearable style="width: 200px;">
            <Option v-for="item in relationshipList" :value="item.value" :key="item.key">{{ item.value }}</Option>
          </Select>
        ...
        <FormItem label="按节点查询" prop="relationship">
          <AutoComplete v-model="keywords" :data="remoteOptions" @on-search="autocompleteSearch" placeholder="请输入导演名称"
    ...
    <Modal :width="50" v-model="modalFlag1" title="新增导演" @on-ok="saveAddDirector" footer-hide :mask-closable="false">
      <Form ref="addForm" :label-width="200" class="form" style="width:80%;" :rules="addRules1" :model="director">
    ...
    <Modal :width="50" v-model="modalFlag2" title="新增演员" @on-ok="saveAddActor" footer-hide :mask-closable="false">
      <Form ref="addForm" :label-width="200" class="form" style="width:80%;" :rules="addRules2" :model="actor">
    ...
    <Modal :width="50" v-model="modalFlag3" title="新增电影" @on-ok="saveAddMovie" footer-hide :mask-closable="false">
      <Form ref="addForm" :label-width="200" class="form" style="width:80%;" :rules="addRules3" :model="movie">
...
<script>
  ...
  export default {
    ...
    methods: {
      saveAddDirector() {
        addDirector(this.director).then(res => {
      ...
      searchHandle() {
        ...
        neo4jRestRxCommit(this.req.relationship).then(res => {
          this.graphData = res.data
          this.init(this.graphData)
        }).catch(err => {
          this.$Message.error('处理异常：' + err)
