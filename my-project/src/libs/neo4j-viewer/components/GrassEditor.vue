...
export default {
  ...
  methods: {
    ...
    updateStyle(selector, styleProp) {
      ...
      this.update(this.graphStyle.toSheet())
    },
    circleSelector(
      styleProps,
      styleProvider,
      ...
      return styleProps.map((styleProp, i) => {
        ...
        return (
          <StyledPickerListItem class={className} key={toKeyString('circle' + i)}>
            <StyledCircleSelector
              class={active ? 'active' : ''}
              style={style}
              onClick={onClick}
        ...
        <StyledInlineListItem key="size-picker">
          <StyledInlineList class="size-picker picker">
            ...
            {this.circleSelector(
              ...
              size => {
                return this.sizeLessThan(size.diameter, styleForLabel.get('diameter'))
              },
              'size-picker-item',
              selector
            )}
    ...
    widthPicker(selector, styleForItem) {
      const widthSelectors = this.graphStyle.defaultArrayWidths().map((widthValue, i) => {
        ...
        return (
          <StyledPickerListItem class="width-picker-item" key={toKeyString('width' + i)}>
            <StyledPickerSelector
              class={active ? 'active' : ''}
              style={style}
              onClick={onClick}
    ...
    captionPicker(selector, styleForItem, propertyKeys, showTypeSelector = false) {
      const captionSelector = (displayCaption, captionToSave) => {
        ...
        return (
          <StyledPickerListItem key={toKeyString('caption' + displayCaption)}>
            <StyledCaptionSelector class={active ? 'active' : ''} onClick={onClick}>
              {displayCaption}
            </StyledCaptionSelector>
          </StyledPickerListItem>
        )
      ...
      const captionSelectors = propertyKeys.map(propKey => {
      ...
      if (this.selectedLabel) {
        ...
        pickers = [
          ...
          this.captionPicker(
            ...
            styleForLabel,
            this.selectedLabel.propertyKeys
          )
        ]
        title = (
          <StyledLabelToken class="token token-label" style={inlineStyle}>
        ...
        pickers = [
          ...
          this.captionPicker(
            ...
            styleForRelType,
            this.selectedRelType.propertyKeys,
            true
          )
        ]
        title = (
          <StyledTokenRelationshipType
            class="token token-relationship"
            style={inlineStyle}
          >
            {this.selectedRelType.relType || '*'}
          </StyledTokenRelationshipType>
