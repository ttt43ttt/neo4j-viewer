...
function Neo4jD3(_selector, _options) {
  var container, graph, info, node, nodes, relationship, relationshipOutline, relationshipOverlay, relationshipText,
  ...
  function appendInfoElement(cls, isNode, property, value) {
    ...
    if (!value) {
      elem.style('background-color', function(d) {
          return options.nodeOutlineFillColor ? options.nodeOutlineFillColor : (isNode ? class2color(property) :
            defaultColor());
        })
        .style('border-color', function(d) {
          return options.nodeOutlineFillColor ? class2darkenColor(options.nodeOutlineFillColor) : (isNode ?
            class2darkenColor(property) : defaultDarkenColor());
        })
  ...
  function appendOutlineToNode(node) {
    return node.append('circle')
      ...
      .style('stroke', function(d) {
        return options.nodeOutlineFillColor ? class2darkenColor(options.nodeOutlineFillColor) : class2darkenColor(d
          .labels[0]);
      })
  ...
  function appendTextToNode(node) {
    return node.append('text')
      ...
      .html(function(d) {
        ...
        var nodeName = d.id;
        // console.log("d.properties.name = " + d.properties.name + ", d.properties[0] = " + d.properties[Object.keys(d.properties)[0]]);
        if (d.properties.name != undefined) {
          nodeName = d.properties.name
        } else if (Object.keys(d.properties)[0] != undefined) {
          console.log('without name property, fetch the first property.');
          nodeName = Object.keys(d.properties)[0]
        }
        return _icon ? '&#x' + _icon : nodeName;
      });
  }
  ...
  function initSimulation() {
    var simulation = d3.forceSimulation()
      ...
      .force('center', d3.forceCenter(svg.node().parentElement.parentElement.clientWidth / 2, svg.node().parentElement
    ...
    data.results.forEach(function(result) {
      result.data.forEach(function(data) {
        ...
        for (var i = 0; i < data.graph.relationships.length; i++) {
          if (i !== 0 && data.graph.relationships[i].source === data.graph.relationships[i - 1].source && data
            .graph.relationships[i].target === data.graph.relationships[i - 1].target) {
  ...
  */
      ...
      outline.attr('d', function(d) {
        var center = {
          ...
          rotatedPointC2 = rotatePoint(center, {
            x: d.target.x - d.source.x - (options.nodeRadius + 1) * u.x - n.x + (n.x - u.x) * options
              .arrowSize,
            y: d.target.y - d.source.y - (options.nodeRadius + 1) * u.y - n.y + (n.y - u.y) * options
              .arrowSize
          }, angle),
  ...
  function unitaryVector(source, target, newLength) {
    var length = Math.sqrt(Math.pow(target.x - source.x, 2) + Math.pow(target.y - source.y, 2)) / Math.sqrt(newLength ||
      1);
